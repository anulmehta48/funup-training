...
const getBooksInYear= async function (req, res) {
   ...
   let savedData=await BookModel.find({year:{$eq:inputYear}})
    res.send({msg:savedData})
}
...
const getXINRBooks= async function (req, res) {
   let savedData=await BookModel.find({"price.indianPrice": {$in:["100INR","200INR","500INR"]}}).select({bookName:1,_id:0})
    res.send({msg:savedData})
}


const getRandomBooks= async function (req, res) {
   let savedData=await BookModel.find({$or:[{stockAvailable:true},{totalPages:{$gt:300}}]})
    res.send({msg:savedData})
}
